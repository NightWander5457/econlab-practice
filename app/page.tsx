...
const graphTasks = [
  ...
  { id: 'perfect-inelastic-supply', title: '画出完全无弹性供给', prompt: 'Draw a perfectly inelastic supply curve.', promptZh: '画出竖直的供给曲线，并使用微观经济坐标轴。', slope: 'vertical', y: 'P', x: 'Q', curve: 'S' },
  { id: 'perfect-elastic-supply', title: '画出完全弹性供给', prompt: 'Draw a perfectly elastic supply curve.', promptZh: '画出水平的供给曲线，并使用微观经济坐标轴。', slope: 'horizontal', y: 'P', x: 'Q', curve: 'S' },
...
const diagramTasks: DiagramTask[] = [
  ...
  { id: 'pes-shape', title: 'PES 曲线陡峭程度', prompt: 'Which supply curve is relatively more price elastic?', promptZh: '哪条供给曲线相对更富有价格弹性？', y: 'P', x: 'Q', curves: [{ path: 'M285 310 L410 65', label: 'S₁', x: 405, y: 72, color: '#183ccc' }, { path: 'M95 260 L590 135', label: 'S₂', x: 580, y: 140, color: '#e0781e' }], options: ['S₂, because quantity responds more to price', 'S₁, because it is steeper', 'Both must have PES = 0'], answerIndex: 0, explanationZh: '在相同坐标尺度下，较平坦的供给曲线表示供给量对价格变化的反应更大。' },
  { id: 'specific-tax-parallel', title: '从量税曲线', prompt: 'Why are S and S + tax parallel?', promptZh: '为什么 S 与 S + tax 两条曲线平行？', y: 'P', x: 'Q', curves: [{ path: 'M115 310 L520 85', label: 'S', x: 515, y: 84, color: '#183ccc' }, { path: 'M115 245 L520 20', label: 'S + tax', x: 493, y: 30, color: '#e0781e' }], options: ['The tax is a fixed amount per unit', 'The tax is a fixed percentage of price', 'Supply is perfectly inelastic'], answerIndex: 0, explanationZh: '从量税每单位税额固定，所以新旧供给曲线的垂直距离相同，两线平行。' },
  { id: 'ad-valorem-tax', title: '从价税曲线', prompt: 'Why does the gap widen at higher prices?', promptZh: '为什么价格越高，两条供给曲线的距离越大？', y: 'P', x: 'Q', curves: [{ path: 'M120 310 L540 90', label: 'S', x: 532, y: 92, color: '#183ccc' }, { path: 'M120 310 L455 25', label: 'S + tax', x: 445, y: 34, color: '#e0781e' }], options: ['The tax is a percentage of the product value', 'The tax per unit is constant', 'Demand becomes more elastic'], answerIndex: 0, explanationZh: '从价税按商品价值的百分比征收，价格越高时每单位税额越大，因此曲线间距扩大。' },
  ...
  { id: 'negative-externality-loss', title: '负外部性福利损失', prompt: 'What does the shaded triangle between Qs and Qm represent?', promptZh: '社会最优产量 Qs 与市场产量 Qm 之间的阴影三角表示什么？', y: 'Cost / Benefit', x: 'Q', area: '300,185 440,110 440,260', curves: [{ path: 'M100 300 L520 70', label: 'MPC', x: 508, y: 75, color: '#8b857d' }, { path: 'M100 245 L520 15', label: 'MSC', x: 508, y: 28, color: '#e0781e' }, { path: 'M100 55 L560 305', label: 'MSB', x: 548, y: 300, color: '#183ccc' }], guides: [{ x1: 300, y1: 185, x2: 300, y2: 340, label: 'Qs', x: 290, y: 366 }, { x1: 440, y1: 110, x2: 440, y2: 340, label: 'Qm', x: 430, y: 366 }], options: ['Welfare loss from over-production', 'Tax revenue', 'Consumer surplus only'], answerIndex: 0, explanationZh: '负外部性下 Qm 高于 Qs，两者之间 MSC 超过 MSB 的区域是过度生产造成的福利损失。' },
  ...
  { id: 'keynesian-lras-read', title: '凯恩斯 LRAS', prompt: 'Why is the curve flat when real output is low?', promptZh: '为什么实际产出较低时，凯恩斯 LRAS 较平坦？', y: 'Price level', x: 'Real output', curves: [{ path: 'M100 285 L390 285 Q520 280 535 55', label: 'LRAS', x: 542, y: 58, color: '#e0781e' }, { path: 'M115 70 L520 300', label: 'AD', x: 510, y: 296, color: '#183ccc' }], options: ['There is substantial spare capacity', 'The economy is already at full capacity', 'Prices can never rise'], answerIndex: 0, explanationZh: '实际产出较低时存在较多闲置资源，总需求增加可主要提高产出，而不必大幅推高价格。' },
  { id: 'ad-left-deflation', title: 'AD 左移与通缩', prompt: 'What is the likely short-run result of AD shifting left?', promptZh: 'AD 向左移的短期结果是什么？', y: 'Price level', x: 'Real output', curves: [{ path: 'M185 75 L585 305', label: 'AD₀', x: 574, y: 300, color: '#8b857d' }, { path: 'M110 75 L510 305', label: 'AD₁', x: 500, y: 300, color: '#e0781e' }, { path: 'M110 305 L530 90', label: 'SRAS', x: 510, y: 92, color: '#183ccc' }], options: ['Lower price level and lower real output', 'Higher price level and higher output', 'Higher price level and lower output'], answerIndex: 0, explanationZh: 'AD左移时，短期均衡价格水平和实际产出都下降，可能带来需求不足和通缩压力。' },
  { id: 'sras-right-productivity', title: '生产率与 SRAS', prompt: 'Productivity rises and SRAS shifts right. What follows?', promptZh: '生产率上升使 SRAS 右移，均衡如何变化？', y: 'Price level', x: 'Real output', curves: [{ path: 'M90 305 L485 90', label: 'SRAS₀', x: 462, y: 92, color: '#8b857d' }, { path: 'M150 305 L545 90', label: 'SRAS₁', x: 522, y: 92, color: '#e0781e' }, { path: 'M110 75 L555 310', label: 'AD', x: 545, y: 304, color: '#183ccc' }], options: ['Lower price level and higher real output', 'Higher price level and lower output', 'Both price and output fall'], answerIndex: 0, explanationZh: '生产率提高降低单位成本，SRAS右移，价格水平下降而实际产出上升。' },
  ...
  { id: 'trend-growth-cycle', title: '实际增长与趋势增长', prompt: 'Actual GDP is above the trend line. Which phase is shown?', promptZh: '实际 GDP 位于长期趋势线上方，表示什么？', y: 'Real GDP', x: 'Time', curves: [{ path: 'M105 300 L565 70', label: 'Trend growth', x: 490, y: 92, color: '#8b857d', dash: '8 5' }, { path: 'M105 295 Q185 345 255 225 Q330 115 400 170 Q485 250 565 55', label: 'Actual GDP', x: 500, y: 58, color: '#e0781e' }], guides: [{ x1: 315, y1: 150, x2: 315, y2: 235, label: 'gap', x: 321, y: 193, color: '#c95d4b' }], options: ['A positive output gap or boom', 'A negative output gap or recession', 'No cyclical movement'], answerIndex: 0, explanationZh: '实际GDP高于趋势或潜在产出时形成正产出缺口，通常对应繁荣与通胀压力。' },
  ...
  return (
    <main className={`app-shell ${focusMode ? 'focus-mode' : ''}`}>
      ...
      {mode === 'records' && <section className="page-card records-page">
        ...
        <div className="source-audit"><strong>✓ 两份讲义已逐页核对</strong><span>U1 24 页 + U2 19 页 · {concepts.length} 个可检查知识点 · {graphQuestionIds.length} 项图形互动</span></div>
